    ...
    pub fn execute(
        ...
        match msg {
            ...
            ExecuteMsg::MintBatch {
                ...
                token_uri,
                extension,
            } => self.mint_batch(deps, info, token_ids, owners, token_uri, extension),
            ExecuteMsg::Approve {
                spender,
                token_id,
                expires,
    ...
    Q: CustomMsg,
{
    ...
    pub fn mint_batch(
        ...
        if token_ids.len() != owners.len() {
            return Err(ContractError::MismatchedIdAndOwnerLengths {});
        }

        let mut claimed_tokens: Vec<String> = Vec::new();
        ...
        for i in 0..token_ids.len() {
            ...
            match self.tokens.update(deps.storage, token_id, |old| match old {
                Some(_) => {
                    claimed_tokens.push(token_id.clone());
        ...
        if !claimed_tokens.is_empty() {
            return Err(ContractError::ClaimedInArray {
                token_ids: claimed_tokens,
            });
        }
        ...
        Ok(Response::new()
            ...
            .add_attribute("owners", format!("{:?}", owners)))
    }
