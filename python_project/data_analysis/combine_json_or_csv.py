...
remaining_addresses_dict = OrderedDict(list(item for item in first_addresses_dict.items() if item[0] is not None) - list(item for item in second_addresses_dict.items() if item[0] is not None))
...
output_json_path = os.path.join(first_json_dir, f"{first_json_basename}_combined_with_{second_json_basename}.json")
output_json_path_2 = os.path.join(first_json_dir, f"{first_json_basename}_removed_of_{second_json_basename}.json")

with open(output_json_path, 'w') as f:
    json.dump(list(combined_addresses_dict.keys()), f, indent=4)

with open(output_json_path_2, 'w') as f:
    json.dump(list(remaining_addresses_dict.keys()), f, indent=4)
